...
ipums_paths <- list(
  "~/Documents/ipumsi_00014.csv",
  "~/Documents/ipumsi_00015.csv",
  "~/Documents/ipumsi_00016.csv"
)
...
ggplot(rice_variability, aes(x = YEAR, y = cv)) +
  geom_line(color = "blue", linewidth = 1.5) +        # updated here
...
names(read_csv("~/Documents/ipumsi_00014.csv"))
...
rajshahi_summary %>%
  select(YEAR, urban_rural, avg_no_toilet_pct)


library(ggplot2)
library(dplyr)

ggplot(rajshahi_summary, aes(x = factor(YEAR), y = avg_no_toilet_pct, fill = urban_rural)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(
    aes(label = paste0(round(avg_no_toilet_pct, 1), "%")),
...
```
This bar chart visualizes the percentage of households without toilets in Rajshahi Division, separated by urban and rural areas across three decades (1991, 2001, 2011).


The chart shows a clear decline in the percentage of households without toilets over time, indicating overall improvements in basic sanitation and poverty reduction in Rajshahi. In 1991, both rural and urban areas had high rates (40% and 38%, respectively). By 2011, these dropped significantly to 12.2% in rural areas and just 9.5% in urban areas. While rural areas consistently lag behind urban ones, the gap has narrowed over time, reflecting more inclusive progress in poverty-related infrastructure.
...
# Saving it
ggsave("poverty_map_bold_borders.png", plot = p, width = 8, height = 6, dpi = 300)
...
names(rajshahi_map)
summary(rajshahi_map$poverty)
sum(!is.na(rajshahi_map$poverty))  # how many valid entries?
...
# Create top_labels: top 15 most poor upazilas
top_labels <- rajshahi_map %>%
  arrange(desc(poverty)) %>%
  slice_head(n = 5) %>%
  st_centroid()

# Plot with only top labels
ggplot(rajshahi_map) +
  geom_sf(aes(fill = poverty), color = "white", size = 0.15) +

  geom_text_repel(
    data = top_labels,
    aes(geometry = geometry, label = ADM3_EN),
    stat = "sf_coordinates",
    size = 3,
    max.overlaps = 100,
    box.padding = 0.3,
...
ggsave("rajshahi_poverty_map.png", width = 8, height = 6, dpi = 300)
...
ggplot(rajshahi_summary, aes(x = avg_price, y = avg_child_dependency, color = urban_rural)) +
  geom_point(size = 3) +
...
ggsave("child_dependency_vs_rice_price.png", width = 8, height = 5, dpi = 300)
...
ggplot(rajshahi_summary, aes(x = avg_price, y = avg_child_dependency, 
...
ggsave("child_dependency_bubble_plot.png", width = 8, height = 5, dpi = 300)
...
# Reshaping for comparison
rajshahi_summary <- rajshahi_summary %>%
  rename(urban_rural = urban_rural.x)
